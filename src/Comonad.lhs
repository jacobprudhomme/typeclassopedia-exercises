...
- Must implement `extract`, but can decide to implement either `duplicate` or `extend`
...
>   extend f s@(Cons _ xs) = Cons (f s) (extend f xs)  -- fmap f (duplicate s)
  ...
  - `duplicate` is like the list function `tails`, which creates a list of all sequential tails of a list
  - `extend` computes a new `Stream` from an old one, where the element at position `n` is computed as a function of everything from position `n` onwards in the old `Stream`
...
- Some more interesting applications or ideas involving comonads:
  ...
  - Relationship of comonads and zippers, using them to create a menu system for a website
